...
if __name__ == "__main__":
    ...
    for t_val in time_stamps:
        ...
        # 打印当前位置和当前模式，方便调试
        print(f"时间: {t_val}, 无人机当前位置: {surgecast_agent.position.copy()}, 当前模式: {surgecast_agent.current_mode}")
    ...
    # 将轨迹数据和其他数据保存到文件（CSV 格式）
    trajectory_df = pd.DataFrame({
        'time': time_stamps[:len(trace_positions)],  # 确保时间戳和轨迹数量一致
        'x_position': [pos[0] for pos in trace_positions],
        'y_position': [pos[1] for pos in trace_positions],
        'wind_speed': wind_speeds,
        'wind_direction': wind_directions,
        'sensor_concentration': sensor_concentrations
    })

    # 保存为 CSV 文件
    trajectory_df.to_csv('drone_trajectory_with_wind_and_concentration.csv', index=False)
    print("轨迹数据已保存到 'drone_trajectory_with_wind_and_concentration.csv'")
    ...
    if search_success_time is not None:
        ...
        print(f"搜索时间: {total_time:.2f}秒，总步长: {total_distance:.2f}米")
