...
def load_plume(
    ...
    data_dir=r'D:\desktop1\2-12-surgecast\plume',  # 确保使用正确的文件夹路径
    ):
    print(dataset)
    puff_filename = os.path.join(data_dir, f'puff_data_{dataset}.pickle')  # 使用os.path.join拼接路径
    wind_filename = os.path.join(data_dir, f'wind_data_{dataset}.pickle')  # 使用os.path.join拼接路径
...
def get_concentration_at_point_in_time_pandas(data, t_val, x_val, y_val):
    """
    计算在给定时间点 (t_val) 和位置 (x_val, y_val) 处的浓度。
    
    :param data: 包含烟羽数据的 DataFrame。
    :param t_val: 当前的时间戳。
    :param x_val: 当前的 x 坐标。
    :param y_val: 当前的 y 坐标。

    :return: 给定时间和位置的浓度总和。
    """
    # 找到所有烟羽源范围与给定的 (x, y, t) 匹配的 puffs
    qx = str(x_val) + ' > x_minus_radius and ' + str(x_val) + ' < x_plus_radius'
    qy = str(y_val) + ' > y_minus_radius and ' + str(y_val) + ' < y_plus_radius'
    q = qx + ' and ' + qy
    
    # 获取数据中的所有时间戳
    time_stamps = data['time'].unique()
    
    # 找到与 t_val 最接近的时间戳
    closest_time = min(time_stamps, key=lambda x: abs(x - t_val))
    
    # 筛选出与最近时间戳匹配的数据
    matching_data = data[data.time == closest_time]
    
    # # 输出匹配到的时间戳
    # print(f"匹配到的时间戳: {closest_time} 对应原时间戳: {t_val}")
    
    # 在符合时间戳条件的子集上进一步查询符合位置范围的数据
    d = matching_data.query(q)
...
# --- 随机轨迹生成函数 ---
def get_random_trajectory(n_pts, xmin, xmax, ymin, ymax, tstart, tend, dt, local_state=None):
...
def plot_puffs(data, t_val, ax=None, show=True):
    ...
    if alphas.size > 0:
        alphas /= np.max(alphas)  # Normalize to 0...1
        alphas = np.power(alphas, 1/8)  # Apply power for better visibility
    else:
        # Handle case when alphas is empty, e.g., set to a default value
        alphas = np.ones_like(data_at_t.x)  # Default to full opacity
...
#     if show:
#         plt.show()
def plot_puffs_and_wind_vectors(data_puffs, data_wind, t_val, ax=None, plotsize=(10,10)):
    """
    绘制烟羽和风速矢量。

    :param data_puffs: 烟羽数据
    :param data_wind: 风速数据
    :param t_val: 当前时间
    :param ax: 当前坐标轴对象
    :param plotsize: 图像大小
    """
    ...
    # 在图中绘制源范围
    source_position = config.source_position  # 假设这是你传入的源的坐标
    success_threshold = config.success_threshold  # 源的半径或某个阈值
    # 绘制烟羽
